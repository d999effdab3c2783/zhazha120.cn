    ...
    protected parsePrimary(): Ast {
        ...
        if (t.tokenType === StringLiteral) {
            try {
                return {
                    type: "literal",
                    value: JSON.parse(t.image.replace(/^'/, '"').replace(/'$/, '"')),
                };
            } catch {
                return {
                    type: "literal",
...
const evaluate = (node: Ast, row: RecordObject): unknown => {
    ...
    switch (node.op) {
        ...
        case "!=":
            return left != right;
        case ">":
            if (isNullish(left) || !isNumber(left) || isNullish(right) || !isNumber(right)) {
                return false;
            }
        ...
        case "~=":
            if (isNullish(left) || !isString(left) || isNullish(right) || !isString(right)) {
                return false;
            }
...
export default <T extends RecordObject>(input: T[], query: string) => {
    ...
    return input.filter((row) => {
        return Boolean(evaluate(ast, row));
    });
};