			...
			<n-card :title="keyAccuracyCalculatorStore.editPreset.name" class="flex-1" size="small">
				<template #header-extra>
					<n-popconfirm @positive-click="keyAccuracyCalculatorStore.resetInputs()">
						<template #trigger>
							<custom-naive-button icon="i-ant-design:delete-outlined" secondary size="small" type="error">清空</custom-naive-button>
						</template>
				...
				<template v-if="isNonNullish(keyAccuracyCalculatorStore.editPreset) && !isEmptyish(keyAccuracyCalculatorStore.editPreset.maps)">
					<custom-naive-vertical-stack>
