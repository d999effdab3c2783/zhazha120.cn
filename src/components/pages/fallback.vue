...
<template>
    <custom-naive-position class="h-screen" placement="center">
        <custom-naive-vertical-stack :size="30" align="center">
            ...
            <custom-vue-bits-fuzzy-text
                :color="themeVars.textColorBase"
                :font-size="isMobile ? 60 : 120"
                >无内容</custom-vue-bits-fuzzy-text
            >

            <template
                v-if="
                    isNonNullish(firstPath) &&
                    isNonNullish($router.resolve({ path: `/${firstPath}` }).name)
                "
            >
            ...
            <custom-naive-button icon="i-tabler:arrow-back" @click="routerNavigation.back"
                >返回</custom-naive-button
            >
        </custom-naive-vertical-stack>
