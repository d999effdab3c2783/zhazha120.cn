<script lang="ts" setup>
    ...
    const item = useDatabase<BirthdayRecord>(async (database) => {
        ...
        return database.birthday_records.where("age").equals(age.value).first();
    });
    ...
    const realMinAgeMessage = computed(() => {
        ...
        return min_age_message.value.replace("{age}", String(age.value));
    });
    ...
    const realMaxAgeMessage = computed(() => {
        ...
        return max_age_message.value.replace("{age}", String(age.value));
    });
</script>
    ...
    <layouts-sub-page>
        ...
        <template v-else>
            <n-card size="small">
                ...
                <template v-else>
                    <n-result
                        :description="`可能是 ${age} 岁生日时并没有发生什么值得记录的事情`"
                        status="404"
                        title="无结果"
                    />
                </template>
