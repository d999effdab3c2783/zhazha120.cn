...
const handle = async () => {
	await props.table.delete(props.record[props.table.schema.primKey.name])
}
</script>
...
<template>
	<n-popconfirm @positive-click="handle">
		<template #trigger>
			<custom-naive-button icon="i-ant-design:delete-outlined" size="small" type="error">删除</custom-naive-button>
		</template>
