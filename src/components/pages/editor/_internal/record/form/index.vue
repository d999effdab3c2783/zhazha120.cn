...
<script lang="ts" setup>
        ...
        const record = Object.fromEntries(
            data.value
                .map((item) => {
                    ...
                    return [item.name, toRaw(item.value)];
                })
                .filter(isNonNullish),
        );
    ...
    const data = ref<Item[]>([
        ...[props.table.schema.primKey, ...props.table.schema.indexes].map((index) => {
        ...
        ...(isNonNullish(props.record)
            ? Object.keys(props.record)
                  .map((name) => {
                      if (
                          [props.table.schema.primKey, ...props.table.schema.indexes].some(
                              (item) => name === item.name,
                          )
                      ) {
                          return;
    ...
    const tableKeys = computed(() => {
        return data.value.map(prop("name")).filter(isNonNullish);
    });
</script>
...
<template>
    <n-form @submit.prevent="handleSubmit">
                ...
                <n-form-item :show-label="false" class="w-full">
                    <n-flex class="w-full" size="small" vertical>
                        ...
                        <template v-if="item.type === 'text'">
                            <text-control
                                :value="item.value ?? ''"
                                @update:value="(newValue) => (item.value = newValue)"
                            />
                        </template>
                        ...
                        <template v-if="item.type === 'blob'">
                            <blob-control
                                ...
                                @upload="(newValue) => (item.value = newValue)"
                            />
                        </template>
        ...
        <n-form-item :show-feedback="false" :show-label="false">
            <custom-naive-button attr-type="submit" icon="i-ant-design:send-outlined"
                >提交</custom-naive-button
            >
        </n-form-item>
    </n-form>
</template>