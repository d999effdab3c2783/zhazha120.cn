<script lang="ts" setup>
    ...
    const { 0: safe_remote_domains } = useZodRegistry(
        z.object({
            "markdown.safe_remote_domains:0": z.array(z.string()),
        }),
    );
    ...
    const isSafeDomain = computed(() => {
        ...
        try {
            ...
            if (![...(safe_remote_domains.value ?? []), location.hostname].includes(host)) {
                return false;
            }
        } catch (e) {
            return false;
    ...
    const raw = computedAsync(async () => {
        ...
        return await (await alova.Get<Response>(props.url).send()).text();
    });
</script>
