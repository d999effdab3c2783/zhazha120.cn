...
const cover = computed(() => {
	return useDatabaseFile(
		useDatabaseValue(props.item, 'cover')
	)
})
</script>
...
<template>
	<n-card size="small">
		...
		<custom-naive-position :class="{ 'text-center': isMobile }" :placement="(isMobile ? 'center' : 'top-left')">
			<custom-naive-auto-stack :size="0" align="center">
				...
				<n-text class="text-5" type="primary">{{ formatDate(item.start_date) }} ~ {{ formatDate(item.end_date) }}</n-text>
		...
		<template v-if="!isEmptyish(item.href)" #action>
			<custom-naive-auto-stack>
				<custom-redirect :href="item.href">
					<custom-naive-button icon="i-ant-design:link-outlined">详情</custom-naive-button>
				</custom-redirect>
			</custom-naive-auto-stack>
		</template>
	</n-card>
