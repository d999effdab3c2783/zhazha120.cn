...
<template>
	<template v-if="isNonNullish(birthday) && isNonNullish(diff)">
		<n-card size="small">
			<custom-naive-position :placement="(isMobile ? 'center' : 'center-left')" class="leading-none">
				<n-text class="text-8">{{ short_name ?? name }} 现在</n-text>

				<custom-naive-horizontal-stack :size="0" :wrap="false" align="baseline" class="font-mono">
