...
import { NCard, NColorPicker, NDivider, NFormItem, NInput, NInputNumber, NSwitch } from 'naive-ui'
import { isEmptyish, isNonNullish, isNullish } from 'remeda'
import type { ModelType } from 'skinview-utils'
import { CrouchAnimation, type EarsLoadOptions, FlyingAnimation, HitAnimation, IdleAnimation, RunningAnimation, SkinViewer, WalkingAnimation, WaveAnimation } from 'skinview3d'
...
const {
	...
	z.object({
		...
		'game.records.minecraft.skinview3d.skin:8': z.string()
			.describe('file'),
		'game.records.minecraft.skinview3d.panorama:9': z.string()
			.describe('file'),
...
const viewer = shallowRef<
	InstanceType<typeof SkinViewer>
>()
...
watch(config, newConfig => {
	...
	if ( isNonNullish(newConfig.visible) ) {
		Object.entries(newConfig.visible)
			.forEach(async ([key, state]) => {
