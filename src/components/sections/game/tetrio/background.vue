...
const {
	...
	z.object({
		'game.records.tetrio.game_background:0': z.string()
			.describe('file'),
		'game.records.tetrio.game_background.overlay:1': z.string()
			.describe('file')
	})
)
</script>
		...
		<n-card size="small" title="背景">
			<n-element class="relative">
				...
				<template v-if="isNonNullish(overlay)">
					<n-image :src="overlay" class="w-1/2 absolute top-1/2 left-1/2 -translate-1/2"/>
				</template>
			</n-element>
			...
			<template #action>
				<custom-naive-auto-stack>
					<custom-redirect :href="background">
						<custom-naive-button icon="i-ant-design:download-outlined">下载</custom-naive-button>
					</custom-redirect>
				</custom-naive-auto-stack>
			</template>
		</n-card>
