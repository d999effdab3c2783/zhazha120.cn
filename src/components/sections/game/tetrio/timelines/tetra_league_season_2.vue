<script lang="ts" setup>
    ...
    defineOptions({
        name: "SectionsGameTetrioTetraLeagueSeason2Timeline",
    });

    const items = useDatabase<TetrioTetraLeagueSeasonTimeline[]>(async (database) => {
        return database.tetrio_tetra_league_season_2_timelines.toArray();
    });
...
<template>
    <template v-if="!isEmptyish(items) && !isEmptyish(mappedItems)">
        <n-card size="small" title="排位赛 / Tetra League - 第 2 赛季 / Season 2 - 段位 | 时间线">
            <custom-naive-horizontal-scroll>
                <n-timeline horizontal>
                    <template v-for="item in mappedItems">
                        <n-timeline-item
                            :time="new Date(item.archived_at).toLocaleDateString()"
                            type="info"
                        >
                            <template #header>
                                <n-popover>
                                    <template #trigger>
                                        <n-image
                                            :img-props="{ class: 'w-6' }"
                                            :src="`https://tetr.io/res/league-ranks/${item.rank}.png`"
                                        />
                                    </template>

                                    <n-text :style="{ color: item.color.value }" class="fw-bold">{{
