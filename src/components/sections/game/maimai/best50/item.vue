...
const cover = computed(() => {
	...
	const realId = (props.item.song_id + offset).toString()
		.padStart(5, '0')
	...
	<n-card :title="(`#${index + 1}`)" class="h-full" size="small">
		...
		<custom-naive-vertical-stack class="h-full">
			<custom-naive-vertical-stack :size="0">
				...
				<n-text type="primary">{{ item.level_label }} {{ item.level }} ({{ item.ds }})</n-text>
				<n-text type="primary">{{ item.rate.replace('p', '+').toUpperCase() }} {{ item.achievements }} -> {{ item.ra }}</n-text>
