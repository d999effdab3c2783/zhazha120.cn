...
const { isLoading, state: data, execute, error } = useAsyncState(async () => {
	if ( isNullish(username.value) ) {
		return
	}

	const result: Data = await (
		await alova.Post<Response>('https://www.diving-fish.com/api/maimaidxprober/query/player', {
			b50: true,
						...
						<custom-naive-vertical-stack>
							<n-card embedded size="small">
								<custom-naive-horizontal-stack align="center">
									...
									<n-tag type="info">{{ sumBy(data.charts.sd, prop('ra')) }}+{{ sumBy(data.charts.dx, prop('ra')) }}={{ data.rating }} Rating</n-tag>
							...
							<n-grid :cols="isMobile ? 1 : 4">
								<template v-for="(item, index) in data.charts.sd">
									<n-grid-item>
										<sections-game-maimai-best50-item :index="index" :item="item"/>
									</n-grid-item>
								</template>
							</n-grid>
