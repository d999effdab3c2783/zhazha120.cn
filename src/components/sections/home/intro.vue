	...
	<custom-naive-position class="h-full" placement="center">
		<custom-naive-auto-stack align="center">
			<template v-if="isNonNullish(avatar)">
				<n-avatar v-motion="motions.avatar" :src="avatar" class="size-30" round @dblclick="handlePoke"/>
			</template>

			<template v-if="isNonNullish(name)">
				<n-element class="transition-([text-shadow] duration-500 ease-in-out) hover:text-shadow-[0_0_.5em]">
