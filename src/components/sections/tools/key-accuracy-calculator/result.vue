...
const filled = computed(() => {
	if ( isNullish(keyAccuracyCalculatorStore.filteredInputs) || isNullish(keyAccuracyCalculatorStore.filteredMaps) ) {
		return
	}

	return keyAccuracyCalculatorStore.filteredInputs.length > 0 && keyAccuracyCalculatorStore.filteredInputs.length === keyAccuracyCalculatorStore.filteredMaps.length
})

const passed = computed(() => {
	if ( isNullish(keyAccuracyCalculatorStore.filteredInputs) || isNullish(keyAccuracyCalculatorStore.result) ) {
		return
	}
...
const share_lines = computed(() => {
	...
	return [
		[
			...
			keyAccuracyCalculatorStore.result.map(value => {
				return value.toFixed(2)
			}).join(' - ')
		].join('\n'),
		[
			`${keyAccuracyCalculatorStore.editPreset.name}`,
			keyAccuracyCalculatorStore.result.map((value, index) => {
...
<template>
	<custom-transitions-fade appear mode="out-in">
		<template v-if="isNonNullish(keyAccuracyCalculatorStore.filteredInputs) && keyAccuracyCalculatorStore.filteredInputs.length > 0 && isNonNullish(keyAccuracyCalculatorStore.result)">
					...
					<custom-naive-position placement="center">
						<custom-naive-vertical-stack align="center">
							...
							<custom-naive-auto-stack align="center">
								<template v-for="(item, index) in keyAccuracyCalculatorStore.result">
									<n-tag :type="(item >= keyAccuracyCalculatorStore.editPreset.require_accuracy ? 'success' : 'error')">{{ item.toFixed(2) }}</n-tag>

									<template v-if="isDesktop && index !== keyAccuracyCalculatorStore.result.length - 1">
									...
									<custom-naive-vertical-stack>
										<template v-for="item in share_lines">
											<n-card embedded size="small">
												...
												<template #action>
													<custom-naive-auto-stack>
														<custom-naive-button icon="i-ant-design:copy-outlined" @click="copy(item)">复制</custom-naive-button>
														<custom-naive-button icon="i-ant-design:copy-outlined" @click="copy( item.split('\n').slice(1).join('\n') )">复制 (无标题)</custom-naive-button>
													</custom-naive-auto-stack>
												</template>
											</n-card>
										</template>
