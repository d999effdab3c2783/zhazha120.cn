...
<template>
	<n-element class="p-2">
		<n-grid :cols="isMobile ? 1 : 5" :y-gap="isMobile ? 12 : 0">
			<n-grid-item>
				<custom-naive-position :placement="(isMobile ? 'center' : 'center-left')" class="h-full">
					<custom-naive-button :focusable="false" :icon="themeStore.nextTheme.icon" :text="isDesktop" size="small" @click="themeStore.switchNextTheme()">{{ themeStore.nextTheme.name }}</custom-naive-button>
			...
			<n-grid-item>
				<custom-naive-vertical-stack :size="0" align="center" class="leading-tight">
					<template v-if="isNonNullish(filing_province_abbreviation) && isNonNullish(filing_icp_code)">
					...
					<template v-if="isNonNullish(filing_province_abbreviation) && isNonNullish(filing_safety_code)">
						<custom-redirect :href="(`https://www.beian.gov.cn/portal/registerSystemInfo?recordcode=${filing_safety_code}`)">
							<n-text type="primary">
								{{ filing_province_abbreviation }}公网安备 {{ filing_safety_code }} 号
							</n-text>
						</custom-redirect>
					</template>
			...
			<n-grid-item>
				<template v-if="isNonNullish(commit_repository)">
					<custom-naive-position :size="0" class="h-full" placement="center">
						...
						<custom-redirect :href="(`https://${commit_repository}/commits/${sha}`)">
			...
			<n-grid-item>
				<custom-naive-position :placement="(isMobile ? 'center' : 'center-right')" class="h-full">
					<sections-utils/>
				</custom-naive-position>
			</n-grid-item>
		</n-grid>
