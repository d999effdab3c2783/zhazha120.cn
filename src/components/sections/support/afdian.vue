	...
	<template v-if="isNonNullish(url)">
		<custom-naive-vertical-stack>
			...
			<n-element class="relative">
				<n-element class="size-full absolute top-0 left-0 transition-([opacity_background] duration-500 ease-in-out) opacity-0 hover:(bg-([#000000] opacity-50) opacity-100)">
					<custom-naive-position class="h-full" placement="center">
						<custom-redirect :href="url">
							<custom-naive-button icon="i-ant-design:link-outlined" type="primary">访问</custom-naive-button>
						</custom-redirect>
