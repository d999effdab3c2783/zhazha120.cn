...
const setupAnimation = () => {
	...
	const enterHandler = (e: MouseEvent) => {
		...
		if ( !target || !cursorRef.value || !cornersRef.value ) {
			return
		}
		...
		const leaveHandler = () => {
			...
			if ( cornersRef.value ) {
				...
				const positions = [
					...
					{ x: cornerSize * 0.4, y: -cornerSize * 1.2 },
					{ x: cornerSize * 0.4, y: cornerSize * 0.4 },
					{ x: -cornerSize * 1.2, y: cornerSize * 0.4 }
				]
...
<template>
	<div
		...
		class="top-0 left-0 z-[120] fixed w-0 h-0 -translate-x-1/2 -translate-y-1/2 pointer-events-none mix-blend-difference transform opacity-0"
	>
		<div
			:style="{ willChange: 'transform' }"
		...
		<div
			:style="{ willChange: 'transform' }"
			class="top-1/2 left-1/2 absolute border-[3px] border-white border-r-0 border-b-0 w-2 h-2 -translate-x-[150%] -translate-y-[150%] target-cursor-corner transform"
		/>
		<div
			:style="{ willChange: 'transform' }"
			class="top-1/2 left-1/2 absolute border-[3px] border-white border-b-0 border-l-0 w-2 h-2 -translate-y-[150%] translate-x-1/2 target-cursor-corner transform"
		/>
		<div
			:style="{ willChange: 'transform' }"
			class="top-1/2 left-1/2 absolute border-[3px] border-white border-t-0 border-l-0 w-2 h-2 translate-x-1/2 translate-y-1/2 target-cursor-corner transform"
		/>
		<div
			:style="{ willChange: 'transform' }"
			class="top-1/2 left-1/2 absolute border-[3px] border-white border-t-0 border-r-0 w-2 h-2 -translate-x-[150%] translate-y-1/2 target-cursor-corner transform"
		/>
	</div>
</template>