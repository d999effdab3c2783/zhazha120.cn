...
export const compressBlob = async (input: Blob, format: CompressionFormat | null) => {
    ...
    return await new Response(input.stream().pipeThrough(new CompressionStream(format))).blob();
};
...
export const decompressBlob = async (input: Blob, format: CompressionFormat | null) => {
    ...
    return await new Response(input.stream().pipeThrough(new DecompressionStream(format))).blob();
};
