...
export const useDatabase = <T>(
    querier: (database: InstanceType<typeof Database>) => unknown,
    options: Partial<{
    ...
    return reference as Readonly<Ref<T>>;
};
...
export const useHasRegistry = (pattern: string) => {
    const regexp = new RegExp("^" + pattern.replaceAll(".", "\\.").replaceAll("*", ".*") + "$");

    return refDefault(
        useDatabase<boolean>(async (database) => {
            return (
                (
                    await database.registry
                        .filter((item) => {
                            return isNonNullish(item.name.match(regexp));
                        })
                        .toArray()
                ).length > 0
            );
...
export const useZodRegistry = <T extends ZodObject>(schema: T) => {
    return Object.fromEntries(
        Object.entries(schema.shape).map(([key, type]: [string, ZodType]) => {
            return [
                key.includes(":") ? key.split(":", 2)[1] : key,
                useDatabase<unknown>(async (database) => {
                    const realKey = key.includes(":") ? key.split(":", 2)[0] : key;

                    const item = await database.registry.where("name").equals(realKey).first();
        ...
        [K in keyof T["shape"] as K extends string
            ? K extends `${string}:${infer S}`
                ? S
                : K
            : never]: Ref<z.infer<T["shape"][K]> | undefined>;
    };
};
...
export const useDatabaseUpdater = () => {
    ...
    const hasData = useDatabase<boolean>(
        async (database) => {
