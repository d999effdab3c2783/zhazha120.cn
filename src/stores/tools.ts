...
import type { AccuracyInput, RawToolsKeyAccuracyCalculatorPreset } from '@/types/components/tools/key-accuracy-calculator'
...
export const useToolsKeyAccuracyCalculatorStore = defineStore('tools.key_accuracy_calculator', () => {
	const toRaw = async (preset: ToolsKeyAccuracyCalculatorPreset): Promise<RawToolsKeyAccuracyCalculatorPreset> => {
		const maps = useDatabase<ToolsKeyAccuracyCalculatorMap[]>(async database => {
			return database.tools_key_accuracy_calculator_maps.where('id')
				.anyOf(preset.map_ids)
				.toArray()
		})

		await until(maps)
			.toMatch(isNonNullish)
	...
	const editPreset = ref<RawToolsKeyAccuracyCalculatorPreset>(
		createRawPreset()
	)
	...
	const editPresetWatcher = watchIgnorable(editPreset, () => {
	...
	const result = computed(() => {
		...
		return filteredInputs.value.map((currentAccuracy, index) => {
			...
			return (currentAccuracy * sumBy(
				filteredMaps.value!.slice(0, index + 1),
				prop('notes')
			) - filteredInputs.value![index - 1] * sumBy(
				filteredMaps.value!.slice(0, index),
				prop('notes')
...
export const useToolsImageWatermarkStore = defineStore('tools.image_watermark', () => {
	const config = ref<
