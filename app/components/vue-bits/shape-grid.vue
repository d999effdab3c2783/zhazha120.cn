...
<script lang="ts" setup>
	...
	const props = withDefaults(
		defineProps<
			Partial<{
				readonly direction: 'diagonal' | 'up' | 'right' | 'down' | 'left'
				readonly speed: number
				readonly borderColor: CanvasStrokeStyle
				readonly squareSize: number
				readonly hoverFillColor: CanvasStrokeStyle
				readonly shape: 'square' | 'hexagon' | 'circle' | 'triangle'
	...
	const gridOffset = ref<Point>({
		x: 0,
		y: 0
	})

	const hoveredSquare = ref<Readonly<Point> | null>(null)

	const trailCells = ref<Readonly<Point>[]>([])
	...
	function initCanvas() {
		...
		const updateCellOpacities = () => {
			...
			if (isNotNil(hoveredSquare.value)) {
		...
		const updateHovered = (col: number, row: number) => {
			if (isNil(hoveredSquare.value) || hoveredSquare.value.x !== col || hoveredSquare.value.y !== row) {
				if (isNotNil(hoveredSquare.value) && props.hoverTrailAmount > 0) {
